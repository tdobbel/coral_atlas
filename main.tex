...
\begin{document}
	...
	\begin{frontmatter}
		...
		\title{Decadal and multispecies \modif{coral} connectivity modeling for \modif{conservation and} restoration prioritization in Florida}
		...
		\affiliation[nova]{
			organization={National Coral Reef Intitute, Nova Southeastern University, United States},
			% addressline={},
			city={Dania Beach},
			% postcode={ },
			state={Florida},
...
%			}
	...
	\section{Larval survival and competency measurements \modif{and models}}\label{appendix:larvae}
	...
	\modif{\begin{table}
		...
		\begin{tabular}{|l|l|}
			...
			\hline
			Exponential mortality rate & $\mu(t) = \lambda$ \\
			\hline
			Weibull mortality rate & $\mu(t) = \lambda\nu(\lambda t)^{\nu-1}$ \\
			\hline
			Exponential competency acquisition rate &  $\alpha(t) = \left\{
				\begin{array}{ll}
					0 & t < t_c, \\
					a & \text{otherwise.}
				\end{array}
			\right.$\\
			\hline
			Exponential competency loss rate & $\beta(t) = b$ \\
			\hline
		\end{tabular}
	\end{table}}
